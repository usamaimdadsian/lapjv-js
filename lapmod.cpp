...
int_t _carr_sparse(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk,
                   const uint_t n_free_rows, int_t *free_rows, int_t *x,
                   int_t *y, cost_t *v) {
  uint_t current = 0;
...
uint_t _find_sparse_1(const uint_t n, uint_t lo, cost_t *d, int_t *cols,
                      int_t *y) {
  uint_t hi = lo + 1;
...
/** Find columns with minimum d[j] and put them on the SCAN list. */
int_t _find_sparse_2(cost_t *d, int_t *scan, const uint_t n_todo, int_t *todo,
                     boolean *done) {
  int_t hi = 0;
  cost_t mind = LARGE;
...
int_t _scan_sparse_2(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk,
                     ...
                     uint_t *pn_todo, int_t *todo, boolean *added, int_t *y,
                     cost_t *v) {
  uint_t lo = *plo;
  uint_t hi = *phi;
...
/**
 * Single iteration of modified Dijkstra shortest path algorithm as explained in
 * the JV paper. This version loops over all column indices (some of which might
 * be inf).
 * @return The closest free column index.
 */
int_t find_path_sparse_1(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk,
                         const int_t start_i, int_t *y, cost_t *v,
                         int_t *pred) {
  uint_t lo = 0, hi = 0;
  int_t final_j = -1;
  uint_t n_ready = 0;
...
/**
 * Single iteration of modified Dijkstra shortest path algorithm as explained in
 * the JV paper. This version loops over non-inf column indices (which requires
...
int_t find_path_sparse_2(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk,
  ...
  while (final_j == -1) {
    ...
    if (final_j == -1) {
      ...
      PRINT_INDEX_ARRAY(scan + lo, hi - lo);
      PRINT_INDEX_ARRAY(todo, n_todo);
      final_j = _scan_sparse_2(n, cc, ii, kk, &lo, &hi, d, pred, done, &n_ready,
                               ready, scan, &n_todo, todo, added, y, v);
      PRINT_COST_ARRAY(d, n);
      PRINT_INDEX_ARRAY(pred, n);
      PRINT_INDEX_ARRAY(done, n);
      PRINT_INDEX_ARRAY(ready, n_ready);
...
/** Find path using one of the two find_path variants selected based on
 * sparsity. */
int_t find_path_sparse_dynamic(const uint_t n, cost_t *cc, uint_t *ii,
                               uint_t *kk, const int_t start_i, int_t *y,
                               cost_t *v, int_t *pred) {
...
int_t _ca_sparse(const uint_t n, cost_t *cc, uint_t *ii, uint_t *kk,
                 const uint_t n_free_rows, int_t *free_rows, int_t *x, int_t *y,
                 cost_t *v, int fp_version) {
  int_t *pred;
